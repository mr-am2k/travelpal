...
const Card = ({ id, fullName, description, age, destination, date, image }) => {
  return (
    <div
      id="travelfeed_post"
      className="w-[80%] md:w-[380px] lg:w-[450px] xl:w-[800px] flex flex-col md:flex-row items-center gap-3 shadow-[0_4px_15px_3px_rgba(0,0,0,0.15)] rounded-[10px] cursor-pointer border-2 border-gray"
    >
      {/* PICTURE */}
      ...
      <div className="w-[90%] h-full flex flex-col gap-3 my-auto py-4">
        ...
        <span className="flex gap-2">
          ...
          <p className="text-[17px] leading-[22px] font-normal">{destination}</p>
        </span>
        <span className="flex gap-2">
        ...
        <span className="flex gap-2">
          ...
          <p className="text-[17px] leading-[22px] font-normal text-slate-700">{description}</p>
        </span>
      </div>
    </div>
  );
